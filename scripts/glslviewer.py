...
class GlslViewer:
    ...
    def __init__(self, name, options=None):
        ...
        if 'frag' in options:
            self.cmd.append(options['frag'])
        else :
            self.cmd.append(name+'.frag')

        if 'geom' in options:
            self.cmd.append(options['geom'])

        if 'vert' in options:
            self.cmd.append(options['vert'])
        ...
        if 'textures' in options:
            ...
            if type(textures) is list:
                ...
                for tex_name in textures.keys():
                    ...
                    self.cmd.append(textures[tex_name])
        ...
        if "exe" in self.options:
            for e in self.options["exe"]:
                self.cmd.append(" -e " + e)
        ...
        if 'noncurses' in options:
            if options['noncurses']:
                self.cmd.append('--noncurses')
        ...
        if 'dynamic_shadows' in options:
            if options['dynamic_shadows']:
                self.cmd.append('-e dynamic_shadows,on')

        if 'cube' in options:
            self.cmd.append('-C ' + options['cube'] )
    ...
    def setDynamicShadow(self, v):
        if not self.isRunning():
            return False
        answer = self.write('dynamic_shadow,' + ('on' if v else 'off'))
        return answer