...
void main(void) {
    ...
    color.rgb = mix(color.rgb, vec3(step(luma(color),0.01)), digits(vec2(st.x, x1f) * vec2(1.,.2) - 0.01, x1i + 1.0, 0.));
    color.rgb = mix(color.rgb, vec3(step(luma(color),0.01)), digits(vec2(st.x, x2f) * vec2(1.,.3) - vec2(0.51, 0.01), abs(x2i) + 1.0, 0.));
