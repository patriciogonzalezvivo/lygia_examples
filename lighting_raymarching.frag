...
#if defined(RAYMARCH_MATERIAL_MAP)
Material raymarchMap( in vec3 pos ) {
    Material res = materialSdf(1.);

    float check = checkBoard(pos.xz, vec2(1.0));

    Material fl = materialSdf( planeSDF(pos) );
    fl.albedo.rgb = vec3( 0.5 + check * 0.5);

    Material Sf = materialSdf( sphereSDF(    pos-vec3( 0.0, 0.60, 0.0), 0.5 ) );
    Sf.albedo.rgb  = vec3( 1.0, 1.0, 1.0);
